...
export const useMcpStore = defineStore('mcp', () => {
  ...
  const callTool = async (toolName: string) => {
    try {
      ...
      console.log("presenter.mcpPresenter.callTool mcp.ts");
      // 调用工具
  ...
  // 安装MCP服务器
  const installMcpServer = async (serverName: string, installData: { command: string; args: string[]; env: Record<string, string> }) => {
    try {
      // 检查服务器是否已存在
      const servers = await mcpPresenter.getMcpServers();
      if (servers[serverName]) {
        // 如果服务器已存在，更新配置
        await mcpPresenter.updateMcpServer(serverName, {
          command: installData.command,
          args: installData.args,
          env: installData.env
        });
      } else {
        // 如果服务器不存在，添加新服务器
        await mcpPresenter.addMcpServer(serverName, {
          command: installData.command,
          args: installData.args,
          env: installData.env,
      ...
      console.error(t('mcp.errors.installServerFailed', { serverName }), error);
      throw error;
    }
  };
