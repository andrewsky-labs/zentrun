...
export class ConversationsTable extends BaseTable {
  ...
  async get(conversationId: string): Promise<CONVERSATION> {
    ...
    return {
      ...
      settings: {
        ...
        userApptoken: result.userApptoken,
  ...
  async update(conversationId: string, data: Partial<CONVERSATION>): Promise<void> {
    ...
    if (data.settings) {
      ...
      if (data.settings.userAppToken !== undefined) {
        updates.push('user_apptoken = ?')
        params.push(data.settings.userAppToken)
      }
