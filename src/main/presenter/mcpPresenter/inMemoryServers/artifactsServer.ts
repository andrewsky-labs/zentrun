...
${ARTIFACT_INSTRUCTIONS_HEAD}
    - HTML: "text/html"
      ...
      - do not forget to add AI information in the footer: "Generated with [Zentrun](https://github.com/Zentrun/zentrun) | All page content is AI-generated and for reference only" ,ensure it matches the page language
${ARTIFACT_INSTRUCTIONS_TAIL}
<example>
...
<body>
    ...
    <footer>
      Generated with <a href="https://github.com/Zentrun/zentrun">Zentrun</a> | All page content is AI-generated and for reference only
    </footer>
</body>
</html>
    </antArtifact>
...
${ARTIFACT_INSTRUCTIONS_HEAD}
    - React Components: "application/vnd.ant.react"
      ...
      - The lucide library 'https://unpkg.com/lucide@latest/dist/umd/lucide.js' is available you can use  like ' <i data-lucide="x"></i>'. but \`lucide.createIcons();\` is already included in the artifact so you don't have to call this.
...
<example>
    ...
    <assistant_response>
      ...
      [React Component Guidelines for Data Visualization]

      1. State Management: Use React.useState() to manage the state of results, errors, and loading status.

      const [results, setResults] = React.useState([]);
      const [error, setError] = React.useState(null);
      const [loading, setLoading] = React.useState(false);

      2. Database Path: Use the agent_slug to construct the database path dynamically. Ensure that the agent_slug is relevant to the current context.

      const agent_slug = "AI-BDR-AGENT-Bsqmj1vUSyrw-QvvMq1-h"; // Ensure this is dynamically set if needed
      const dbPath = \`/home/dslabglobal/.config/Zentrun/app_db/\${agent_slug}.db\`;

      3. SQL Query: Write a SQL query that fits the context of the data and you want to visualize. For example, to fetch the top 10 posts based on comments from posts_reddit table (table and columns can be changable and be relevant to the current context.):

      const query = "SELECT title, comments FROM posts_reddit ORDER BY comments DESC LIMIT 10"; // Adjust this query as necessary

      4. Database Query Function: Use an asynchronous function to query the SQLite database using the path and query you defined. Handle loading state and errors appropriately.
      ...
      5. Effect Hook: Use React.useEffect() to call the queryDatabase function when the component mounts.
      ...
      6. Data Visualization: Choose an appropriate chart type (bar chart, line chart, etc.) based on the data being visualized. Use the Recharts library to create the chart.

      7. Display Fetched Data: Include a section to display the fetched data in a readable format for debugging purposes, such as using a <pre> tag to format the JSON output.

      8. Component Naming: Ensure the component is named App and that you use ReactDOM.createRoot() for rendering instead of ReactDOM.render().

      9. Avoid Imports: Do not use import statements for React or other libraries, as they are already available in the environment. Use the existing libraries directly.

      10. When working with React artifacts, remember that all hooks must follow React's Rules of Hooks:
      10-1. Only call hooks at the top level of your component
      10-2. Only call hooks from React function components or custom hooks
      10-3. Don't call hooks inside loops, conditions, or nested functions

      - When generating HTML or JavaScript for displaying content in a sandboxed environment (e.g., within \`iframe\` or \`srcdoc\`), follow these rules:

      1. **Never use local file URLs (e.g., \`file://\`)** for image or media sources. Instead:
         - Convert images to Base64 and use the \`data:\` URI format.
         - Or, provide instructions to serve the file via a local HTTP server (e.g., \`http://localhost:PORT/...\`).

      2. **To allow opening external links in new tabs from sandboxed iframes**, always:
         - Add \`sandbox="allow-scripts allow-popups"\` to any \`<iframe>\` if external links are intended to be opened.
         - Or recommend opening links outside of the iframe context (e.g., through a dedicated function call).

      If such limitations cannot be avoided, clearly mention the limitation and suggest an alternative or workaround in the generated output.
...
export class ArtifactsServer {
  ...
  constructor() {
    ...
    this.server = new Server(
      {
        name: 'zentrun-inmemory/artifacts-server',
        version: '1.0.0'
      },
      {
        capabilities: {
