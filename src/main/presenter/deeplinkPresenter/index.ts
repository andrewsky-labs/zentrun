 ...
 * DeepLink 处理器类
 * 负责处理 zentrun:// 协议的链接
 * zentrun://start 唤起应用，进入到默认的新会话界面
 * zentrun://start?msg=你好 唤起应用，进入新会话界面，并且带上默认消息
 * zentrun://start?msg=你好&model=deepseek-chat 唤起应用，进入新会话界面，并且带上默认消息，model先进行完全匹配，选中第一个命中的。没有命中的就进行模糊匹配，只要包含这个字段的第一个返回，如果都没有就忽略用默认
 * zentrun://mcp/install?json=base64JSONData 通过json数据直接安装mcp
 */
  ...
  init(): void {
    ...
    if (process.defaultApp) {
      if (process.argv.length >= 2) {
        app.setAsDefaultProtocolClient('zentrun', process.execPath, [
          path.resolve(process.argv[1])
        ])
      }
    } else {
      app.setAsDefaultProtocolClient('zentrun')
    }
    console.log("DeeplinkPresenter");
    // 处理 macOS 上协议被调用的情况
    app.on('open-url', (event, url) => {
      ...
      if (!app.isReady()) {
        ...
        console.log('open-url ready URL:', url)
    ...
    eventBus.on(WINDOW_EVENTS.READY_TO_SHOW, () => {
      if (this.startupUrl) {
        console.log('open-url process startup URL:', this.startupUrl)
    ...
    if (!gotTheLock) {
      ...
      app.on('second-instance', (_event, commandLine) => {
        const deepLinkUrl = commandLine.find((arg) => arg.startsWith('zentrun://'))
        ...
        if (process.platform === 'win32') {
          ...
          if (deepLinkUrl) {
            if (!app.isReady()) {
              console.log('second-instance URL:', deepLinkUrl)
              this.startupUrl = deepLinkUrl
            } else {
              console.log('second-instance ready URL:', deepLinkUrl)
