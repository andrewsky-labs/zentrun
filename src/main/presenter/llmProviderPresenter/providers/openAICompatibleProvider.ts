...
export class OpenAICompatibleProvider extends BaseLLMProvider {
  ...
  constructor(provider: LLM_PROVIDER, configPresenter: ConfigPresenter) {
    ...
    } else if (provider.id === 'zentrun') {
      // For Zentrun provider, use the auth token instead of API key
      const authToken = this.configPresenter.getAuthToken()
      this.openai = new OpenAI({
        apiKey: authToken || this.provider.apiKey, // Fallback to API key if no auth token
        baseURL: this.provider.baseUrl,
        httpAgent: proxyUrl ? new HttpsProxyAgent(proxyUrl) : undefined,
        defaultHeaders: {
          ...this.defaultHeaders
