...
// API base URL for version check
// const API_BASE_URL = 'http://0.0.0.0:2502'
const API_BASE_URL = 'https://api.zentrun.com'
...
const getVersionCheckBaseUrl = () => {
  return 'https://api.zentrun.com/check-version'
}
...
export class UpgradePresenter implements IUpgradePresenter {
  ...
  private _apiVersionChecked: boolean = false // 标记是否已经检查过API版本
  private _needToUpdate: boolean = false // 标记是否需要强制更新
  ...
  constructor() {
    ...
    autoUpdater.on('error', (e) => {
      ...
      eventBus.emit(UPDATE_EVENTS.STATUS_CHANGED, {
        ...
        needToUpdate: this._needToUpdate
      })
    })
  ...
  async checkApiVersion() {
    ...
    // 如果已经检查过API版本，则不再检查
    if (this._apiVersionChecked) {
      return
    }

    try {
      console.log('检查API版本')
      const response = await axios.get(`${API_BASE_URL}/zentrun-version/`)
      const apiVersion = response.data
      const currentVersion = app.getVersion()

      console.log('API版本:', apiVersion)
      console.log('当前版本:', currentVersion)

      // 如果API返回的版本比当前版本高，并且needToUpdate为true，则需要强制更新
      if (apiVersion.version && apiVersion.needToUpdate === true) {
        if (compare(apiVersion.version, currentVersion, '>')) {
          console.log('需要强制更新')
          ...
          this._versionInfo = {
            ...
            releaseDate: new Date().toISOString().split('T')[0], // 使用当前日期
            releaseNotes: '新版本已发布，请更新后使用。',
            githubUrl: '',
            downloadUrl: ''
          }
